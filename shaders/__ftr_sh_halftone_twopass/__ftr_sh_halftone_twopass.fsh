...
void main() {
	...
	vec2 dir = direction;
	if (progress > 0.5) dir *= -1.0;
	vec2 center = vec2(0.5);
	vec2 v = normalize(dir);
	v /= abs(v.x) + abs(v.y);
	
	float d = v.x * center.x + v.y * center.y;
	float hf = smoothstep(-smoothness, 0.0, (v.x * uv2.x + v.y * uv2.y) - (d-0.5+pw * (1.0+smoothness)));
	if (invert) hf = 1.0 - hf;
	...
	float halftone = (1.0 - step(pw, 0.0)) *
	...
	gl_FragColor = vec4(mix((progress < 0.5) ? col_tex2.rgb : col_tex.rgb, color, halftone), 1.0);
}
