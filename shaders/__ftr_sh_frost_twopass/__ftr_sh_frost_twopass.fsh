...
void main() {
	...
	float circ = smoothstep(inner, inner*10.0, length(uv - 0.5))*pw;
	...
	vec3 frost = mix(vec3(1.0), color_frost, circ+pw*frost_multiplier);
	gl_FragColor = col_blur * vec4(color, 1.0) * vec4(frost /= 1.0-vec3(pw), 1.0);
}
